...
date
echo -----------------------------
echo LEVEL 2 WORKSTATION AUDIT
echo
...
level2w | tr '\t' ',' | tr -d '[' | sed -e 's/91m//g' | sed -e 's/92m//g' | sed -e 's/0m//g' >> all_audits.csv
level2w | tr '\t' ',' | tr -d '[' | sed -e 's/91m//g' | sed -e 's/92m//g' | sed -e 's/0m//g' > latestresult.csv
sleep 2

echo -ne '###############               [60% COMPLETE]\r'

level2w > latestresult.txt
